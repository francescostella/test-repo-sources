...
gulp.task('build:prod', ['build'], function(cb) {
  exec('cd dist/ && git add . && git commit -m "Build by CircleCI" && git config --global user.email "circleci@circleci.com" && git config --global user.name "CircleCI" && git push -u origin master', function (err, stdout, stderr) {
    console.log(stdout);
    console.log(stderr);
    cb(err);
  });
